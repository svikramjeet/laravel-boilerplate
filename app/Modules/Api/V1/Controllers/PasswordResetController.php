...
class PasswordResetController extends ApiController
{
        ...
        if (! $user) {
            $this->showBadRequestError(
                ['email' => __('messages.password.not_found')],
                 __('messages.user.not_found'), 400);
        }
        ...
        if (! $password_reset) {
            $this->showBadRequestError(
                ['token' => __('messages.user.token_expired')],
                 __('messages.error.validation'), 404);
        }
    ...
    public function reset(Request $request)
    {
        ...
        return $this->showSuccessRequest(['email' => $this->input['email']],
                                         __('messages.password.update_success'), 200);
    }
